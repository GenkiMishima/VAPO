...
if __name__ == "__main__":
	...
	for j in range(1,Pres_count):
		...
		AllPres = np.append(AllPres,P)
		AllTemp = np.append(AllTemp,float(Temp))
		AllMdot = np.append(AllMdot,Mdot_th)
	...
	plt.plot(AllMdot,AllPres*10**(-1.0))
	plt.grid()
	#plt.title('MdotO:%s[kg/s]'%MdotO)
	plt.xlabel('Mdot_th[kg/s]')
	plt.ylabel('Pres[MPa]')
	#plt.ylim([0,100])
	plt.savefig("MdotCheck_%s.png"%of)
	plt.close()
	plt.plot(AllMdot,AllTemp)
	plt.xlabel('Mdot_th[kg/s]')
	#plt.savefig("MdotCheck_%s.png"%of)
	plt.show()
